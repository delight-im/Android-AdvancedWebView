...
public class AdvancedWebView extends WebView {
	...
	protected void setGeolocationDatabasePath() {
		...
		if (mFragment != null && mFragment.get() != null &&
				Build.VERSION.SDK_INT >= 11 && mFragment.get().getActivity() != null) {
	 ...
	 */
	public void loadHtml(final String html, final String baseUrl,
						 final String historyUrl, final String encoding) {
	...
	protected void init(Context context) {
		...
		super.setWebViewClient(new WebViewClient() {
			...
			// TODO: figure out what to do with this so that we either
			// 1) don't have to remove it when using over api 23 by using some workaround or something, or
			// 2) just remove it altogether, and see what effects that would have when being used in an app
	...
	public boolean isPermittedUrl(final String url) {
		...
		if (!actualHost.matches("^[a-zA-Z0-9._!~*')(;:&=+$,%\\[\\]-]*$")) {
			// prevent mismatches between interpretations by `Uri` and `WebView`, e.g. for
		...
		if (actualUserInformation != null &&
				!actualUserInformation.matches("^[a-zA-Z0-9._!~*')(;:&=+$,%-]*$")) {
			// prevent mismatches between interpretations by `Uri` and `WebView`, e.g. for
			// `http://evil.example.com\@good.example.com/`
			return false;
		}
